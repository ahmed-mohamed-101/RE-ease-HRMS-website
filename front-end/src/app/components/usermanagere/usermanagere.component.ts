...
export class UsermanagereComponent {
constructor(private _UserredataService:UserredataService,private _Router:Router, private _EmployeesService:EmployeesService,
...
RE: any = [];
showRE() {
  this._EmployeesService.showRE(this.token).subscribe({
    next: (response) => {
      this.RE = response;
      console.log(response);
    },
...
this.userSearch={
token:this.token,
search:this.searchRE
}

this._EmployeesService.searchRE(this.userSearch).subscribe({
next:(response)=>{
console.log(response);
this.RE=response
...
// changeStatus
changeStatus(id: any) {
  this._EmployeesService.changeStatusRe(id, this.token).subscribe({
    next: (response) => {
      console.log(response);
      this.showRE();
